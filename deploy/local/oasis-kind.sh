...
# Delete this file and re-run \`secrets\` to rotate; the pods need a
# \`rollout restart\` afterwards to pick the new values up.
...
cmd_wait() {
  ...
  info "Migrations"
  # Wait on both outcomes at once. Waiting only for 'complete' would sit
  # through the whole timeout on a Job that already gave up.
...
cmd_cluster_down() {
  ...
  note "${SECRETS_FILE#"${REPO_ROOT}/"} is kept; delete it to rotate the local secrets."
}

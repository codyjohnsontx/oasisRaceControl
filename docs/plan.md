...
`RIG_HEARTBEAT`, `IRACING_SESSION_STARTED`, `LAP_COMPLETED`, `IRACING_SESSION_ENDED`, `DRIVER_SIGNED_OUT` (from agent UI), `RIG_IDLE_WARNING/RIG_IDLE_TIMEOUT`. Every event carries `eventId` (UUID minted when queued), rig identity (from token), and the active `rigAssignmentId` **as known by the agent at capture time** — the server attributes from that stamp and never from whatever assignment happens to be open when the batch arrives.

The stamp has three states and the server treats each differently. A **uuid** is attributed to that assignment, scoped to the calling rig; whether the assignment has since closed is deliberately irrelevant, because a lap that waited out a network outage still belongs to the driver who drove it and refusing it there would destroy the backlog the durable outbox exists to protect (`accepted` / `accepted_invalid`). An **explicit null** means the agent knew nobody was checked in: refused as `no_active_assignment`. An **absent key** means the agent predates the field and cannot say: refused as `attribution_unsupported`, so an older agent degrades to sending nothing rather than quietly inheriting the next driver. A uuid this rig has never owned is refused as `unknown_assignment`. None of the three refusals is settled by the agent, so the lap stays in its outbox rather than being destroyed — see *unattributable laps* below. `LAP_COMPLETED` payload mirrors the spec's illustrative JSON; exact iRacing fields are finalized after the spike. Server-side validity check re-runs independently (combo match vs tonight's/challenge config, incident_delta > 0 → `INCIDENT_LIMIT_EXCEEDED`, duplicate `event_id` → dropped).
...
- **Unattributable laps** (driven with nobody checked in, or sent by an agent too old to stamp them) are refused and left in the agent's outbox. They are never credited to the next driver to check in, and never dropped on the floor. Whether they should ultimately be discarded, stored unattributed, or surfaced for staff to attribute is an open product decision; until it is made the outbox is the holding pen, which is safe but unbounded across a long unattended run.

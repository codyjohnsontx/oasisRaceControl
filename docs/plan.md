...
- **Known limitation, bounded by design:** once the agent has polled successfully it keeps stamping from its last known assignment across failed polls, so if it is offline while the seat changes hands - a checkout or takeover from a phone, both invisible to an offline agent - the next laps carry the departed driver's assignment. The server's window check bounds this rather than preventing it: such a lap is refused once it is more than 15 minutes past that assignment's `ended_at`. Closing it would need a cloud-to-rig push, which the polling-only design does not have.

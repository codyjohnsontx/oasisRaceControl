...
- `laps` — id, event_id (unique — idempotency), rig_assignment_id, driver_id (both null together on an unattributable lap; see the event model),  sim_session_id, track/config/car denormalized, challenge_id (nullable, resolved server-side), lap_number, lap_time_ms (int), incident_delta, is_valid, invalid_reason (enum from spec), unattributed_cause (enum; set exactly when driver_id is null), completed_at. **Immutable except is_valid/invalid_reason via staff action.**
...
- **A lap the server refuses** (400 `invalid_input` - an over-bound `lapTimeMs`, or any other malformed field, since the body is validated whole) is **quarantined by the agent**, not retried. The response's zod issue paths name the offending events by their position in the batch; the agent parks exactly those rows in its SQLite outbox with the reason the server gave, logs each one once, and the next flush carries the rest of the queue. The lap is kept, never re-sent, and shown on the rig's status line apart from the queued count, because it is waiting for a person rather than for the link to come back. Only a refusal that names events is treated this way: a 401 from a rotated rig token, a 429, or a proxy's error page names no lap, so those still count as the backend being unreachable and every lap is retried, which is what keeps a bad token from quietly retiring a night's driving. Before this, any non-2xx threw, the agent could not tell it from the venue's network being down, and the next flush re-sent the same oldest-first batch every five seconds - one rejected lap held every lap queued behind it. That case is reachable with real telemetry, not only with corrupt data: iRacing's `LapLastLapTime` includes pit-box time on an in-lap, so a driver who parks for half an hour produces a genuine over-bound lap.

...
- **Known limitation, agent side:** a batch the server rejects with a 400 (`invalid_input` - an over-bound `lapTimeMs`, or any other malformed field, since the body is validated whole) is indistinguishable to the agent from the network being down: `BackendClient.SendLapsAsync` throws on any non-2xx, `AgentService.RunBackend` marks the rig offline, and the next flush re-sends the same oldest-first batch, so one rejected lap holds every lap queued behind it. No telemetry source can produce such a lap today. The fix belongs in the agent - quarantine the events the response's issue paths name and resend the rest - and is tracked as `oasis-agent-quarantine-rejected-events`, not here.

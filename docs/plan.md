...
The stamp has three states. A **uuid** is attributed to that assignment, scoped to the calling rig; whether the assignment has since closed is deliberately irrelevant, because a lap that waited out a network outage still belongs to the driver who drove it and refusing it there would destroy the backlog the durable outbox exists to protect (`accepted` / `accepted_invalid`). An **explicit null** means the agent knew nobody was checked in. An **absent key** means the agent predates the field and cannot say — not the same thing, and never treated as one, so an older agent degrades to recording nothing rather than quietly inheriting the next driver. A uuid this rig has never owned is a fourth, abnormal case.

The last three are all *unattributable*, and all three are stored the same way: `accepted_unattributed` — a real lap row with no `driver_id` and no `rig_assignment_id`, invalid with reason `UNATTRIBUTED`. The two abnormal causes (an agent that sends no id, an id this rig has never owned) are logged once per batch, because both mean somebody has to go fix something. `accepted_unattributed` is a settling status, so the agent drops the lap from its outbox: the venue keeps the lap, and a rig nobody ever checks into cannot fill its own disk. `LAP_COMPLETED` payload mirrors the spec's illustrative JSON; exact iRacing fields are finalized after the spike. Server-side validity check re-runs independently (combo match vs tonight's/challenge config, incident_delta > 0 → `INCIDENT_LIMIT_EXCEEDED`, duplicate `event_id` → dropped).

...
3. **Environment Variables:**
   ...
   within 2s, or 200 carrying the applied-migration count. `/api/health` only
   says the process is up and never touches the database, so it stays 200
   through a bad `DATABASE_URL` by design. A missing `SESSION_SECRET` still
   surfaces only on the first sign-in.
